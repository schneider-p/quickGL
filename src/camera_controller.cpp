...
void CameraController::onClick(int button, int state, int x, int y) {
  //	cout << "Clicked by Tool::CameraTransformer at: " << x << " " << y <<
  //endl;
...
void CameraController::onMouseMove(int x, int y) {
  ...
  //	}
  if (mClick || rClick) {
    Shape::activeCamera->ty -=
        (1 / 250.f) * (y - y0); // -= because y is measured from top
...
//	InteractiveCamera(glm::vec3 _position, glm::vec3 _lookingAt, glm::vec3
//_Up) : Tool() { 		cout << "Constructing ICam" << endl; 		cam = new
//Camera(_position, _lookingAt, _Up);
//	}
