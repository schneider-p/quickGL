...
// WORKS for 2D and 3D
vector<float> Palette::mapValues(float *v, int nval, int stride, int offset,
  ...
  //	cout << "nCol = " << nCol << ", colMax = " << colMax << ", colMin = " <<
  //colMin << '\n';
  for (int i = 0; i < nval; ++i) {
    glm::vec4 c;
    int colID =
        (v[stride * i + offset] - min_val) / (max_val - min_val) * (n - 1);
...
// WORKS for 2D and 3D
vector<unsigned char> Palette::mapValues_byte(float *v, int nval, int stride,
                                              int offset, float vmin,
                                              float vmax) {
