...
Tool::Tool() {
  ...
  cout << "Constructing tool #" << activeTools.size() << ", "
       << activeTools.size() << " in list" << endl;
}

Tool::~Tool() {
  try {
    list<Tool *>::iterator it =
        find(activeTools.begin(), activeTools.end(), this);
    if (it == activeTools.end())
      throw string("Tool destructor could not find tool in active list!");
    activeTools.erase(it);
    cout << "Destroying tool, " << activeTools.size() << " left in list."
         << endl;
  } catch (string s) {
...
void Tool::activate() {
  ...
  try {
    ...
    if (it == activeTools.end())
      throw string("Tool destructor could not find tool in active list!");
    activeTools.splice(
        activeTools.begin(), activeTools,
        it); // erase *it from current location (it) and put it at begin()
  } catch (string s) {
    cout << "FATAL ERROR: " << s << endl;
  }
}
