...
Shape::Shape(int nverts, GLenum mode) {
  ...
  glTexImage2D(
      ...
      pixels); // After reading one row of texels, pointer advances to next 4
               // byte boundary. Therefore ALWAYS use 4byte colour types.
  ...
  // sort the shapes by distance to camera, so that transparency is handled
  // correctly. We are just sorting shapes, not individual triangles, and
  // praying to god that thats good enough! Sort shapes list so that newly
  // inserted shape gets into the correct location. allShapes.sort([](Shape*
  // &s1, Shape* &s2){ return min(s1->bbox0.z,s1->bbox1.z) <
  // min(s2->bbox0.z,s2->bbox1.z);}); // FIXME: sort by distance from camera
  // rather than absolute z
  if (activeCamera != NULL)
    activeCamera->sortShapes();
}
...
Shape::~Shape() {
  ...
  try {
    ...
    if (it == allShapes.end())
      throw string("Shape destructor could not find shape in global list!");
    allShapes.erase(it);
  } catch (string s) {
...
void Shape::setVertices(const float *verts) {
  ...
  bbox0 = accumulate((glm::vec3 *)verts, (glm::vec3 *)verts + nVertices,
                     glm::vec3(1e20f, 1e20f, 1e20f),
                     [](const glm::vec3 &p1, const glm::vec3 &p2) {
                       return glm::vec3(min(p1.x, p2.x), min(p1.y, p2.y),
                                        min(p1.z, p2.z));
                     });

  bbox1 = accumulate((glm::vec3 *)verts, (glm::vec3 *)verts + nVertices,
                     glm::vec3(-1e20f, -1e20f, -1e20f),
                     [](const glm::vec3 &p1, const glm::vec3 &p2) {
                       return glm::vec3(max(p1.x, p2.x), max(p1.y, p2.y),
                                        max(p1.z, p2.z));
                     });
...
void Shape::setElements(int *ele, int nelements) {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, nelements * sizeof(int), ele,
               GL_DYNAMIC_DRAW);
}
...
void Shape::applyTexture(float *uvs, unsigned char *pixels, int w, int h) {
  ...
  glBufferData(GL_ARRAY_BUFFER, 2 * nVertices * sizeof(float), uvs,
               GL_DYNAMIC_DRAW);
  CHECK_GL_ERROR();
...
void Shape::render() {
  ...
  if (useElements)
    glDrawElements(renderMode, nElements, GL_UNSIGNED_INT, (void *)0);
  else
